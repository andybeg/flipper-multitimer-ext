...
#define TIMER_STORAGE_VERSION        6
...
static void load_timer_data(MultiTimerApp* app) {
    ...
    if(storage_file_open(file, TIMER_DATA_FILE, FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        if(app->timer_storage.magic != TIMER_STORAGE_MAGIC ||
           (app->timer_storage.version != TIMER_STORAGE_VERSION &&
            app->timer_storage.version != TIMER_STORAGE_V5 &&
            app->timer_storage.version != TIMER_STORAGE_V4 &&
            app->timer_storage.version != TIMER_STORAGE_V3 &&
            app->timer_storage.version != TIMER_STORAGE_V2 &&
            app->timer_storage.version != TIMER_STORAGE_LEGACY_VERSION)) {
            // Invalid data, reset
            memset(&app->timer_storage, 0, sizeof(TimerStorage));
        } else if(app->timer_storage.version == TIMER_STORAGE_VERSION) {
        ...
        if(app->timer_storage.version != TIMER_STORAGE_VERSION) {
            memset(
...
static Epaper42TimerState multitimer_epaper_state(TimerState state) {
    switch(state) {
    case TimerStateRunning:
        return Epaper42TimerStateRunning;
    case TimerStatePaused:
        return Epaper42TimerStatePaused;
    case TimerStateFinished:
        return Epaper42TimerStateFinished;
    case TimerStateSetup:
    default:
        return Epaper42TimerStateStopped;
    }
}
...
static void multitimer_epaper_update(MultiTimerApp* app, bool force) {
    ...
    epaper42_bw_show_timer(
        ...
        multitimer_epaper_state(timer->state),
        app->epaper_partial_mode ? Epaper42RefreshModePartial : Epaper42RefreshModeFull,
        app->epaper_invert_colors);
