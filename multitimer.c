...
#define TIMER_STORAGE_VERSION        7
...
static void load_timer_data(MultiTimerApp* app) {
    ...
    if(storage_file_open(file, TIMER_DATA_FILE, FSAM_READ, FSOM_OPEN_EXISTING)) {
        ...
        if(app->timer_storage.magic != TIMER_STORAGE_MAGIC ||
           (app->timer_storage.version != TIMER_STORAGE_VERSION &&
            app->timer_storage.version != TIMER_STORAGE_V6 &&
            ...
            if(app->timer_storage.version == TIMER_STORAGE_VERSION &&
               app->timer_storage.epaper_model < EpaperModelCount) {
...
static void multitimer_epaper_show_stopped(MultiTimerApp* app, uint32_t duration_seconds) {
    ...
    epaper_show_timer(
        ...
        app->epaper_partial_mode ? EpaperRefreshModePartial : EpaperRefreshModeFull,
